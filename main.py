...
async def check(ctx):
    if ctx.message.attachments:
        for attachment in ctx.message.attachments:
            ...
            await ctx.send(get_class(model_path="./converted_keras2sampah/keras_model.h5", labels_path="./converted_keras2sampah/labels.txt", image_path=f"./{attachment.filename}"))
    else:
